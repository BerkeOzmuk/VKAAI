...
"""
	We maken eerst gebruik van een dense laag van 50 units en een sigmoid activatie functie. 
	Dit doen we omdat 50 units niet te laag was om slechte resultaten te geven maar ook niet te hoog om overfitting te voorkomen.
	We gebruiken de sigmoid functie omdat deze handig is voor het voorspellen van de output.
	
	Hierna maaken we gebruik van een andere dense laag van 10 units en een softmax activatie functie.
	Dit doen we omdat 10 units overeen komt met de 10 mogelijk getallen die je kan krijgen.
	We gebruiken de softmax functie omdat deze functie voornamelijk wordt gebruikt voor de laatste laag bij classificatie.
"""
...
"""
	Het aantal parameters is het aantal inputs plus 1 keer het aantal units. 
	Dan kom je dus uit op:

	(784 + 1) * 50 = 39250 voor de eerste layer
	(50 + 1) * 10 = 510 voor de tweede layer
"""
...
if (bInitialiseWeightsFromFile):
    model.load_weights("myWeights.weights.h5"); # let's continue from where we ended the previous time. That should be possible if we did not change the model.
...
"""
	1. De eerste foto is de foto van het getal wat geclasifiseert moet worden.
	   De tweede foto met de blokjes stellen de verschillende cijfers voor.
	   Van links naar recht is het 0 tot en met 9.
	   Het witte blokje stelt voor welk cijfer het netwerk toekent aan de eerste foto.
	2. Hier zie je zoals bij 1 de eerste foto van het cijfer gevolgd door blokjes.
	   Hierbij zijn er 50 blokjes, deze stellen de 50 units van de dense layer voor.
	   De witte blokjes stellen de units die geactiveerd zijn voor.
	3. Precies hetzelefde als wat er bij 1 is uitgelegd.
"""
