...
"""
	Er zijn twee soorten plaatjes.
	Het eerste soort plaatje is de largeTestImage.
	Dit plaatje bevat random cijfers uit de dataset op random plekken.
	Het tweede soort plaatje zijn de FeaturemapOfLargeTestImage plaatjes.
	Deze plaatjes bevatten wat de outputlaag classificeerd als een getal.
	Elk plaatje is voor een ander getal, daarom zijn er tien plaatjes.

	De pooling lagen verkleinen de input shape van de afbeeldingen.
...
"""
	Als eerste is het handig om elke waarde een threshold te geven.
	Hierna geef je alle opvallende features een label.
	Als laatste kan je bounding boxes om alle gelabelde features tekenen.
"""
...
"""
	...
	Conv2D 1	30 * (3 * 3) * (1024 * 1024) = 283115520 
	Conv2D 2	60 * (3 * 3) * (1024 * 1024) = 566231040
	Conv2D 3	120 * (3 * 3) * (1024 * 1024) = 1.1324620E9
	Conv2D 4	10 * (1 * 1) * (1024 * 1024) = 10485760 

	Een Teraflop is 1 biljoen floating point operaties per seconde.
	Er zijn in totaal 1.9922944E9 operaties.
	Dus zou het 1.9922944E9 gedeeld door 1 biljoen seconden moeten zijn.

	Maar het zal natuurlijk langer duren omdat dit de meest ideale situatie is.
	In het echt moet een GPU ook veel andere dingen doen, dus zal het waarschijnlijk een stuk langer duren.
"""
...
optimizer = keras.optimizers.Adam(learningrate) #lr=0.01 is king
